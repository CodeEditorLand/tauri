...
import { PhysicalPosition, type LogicalPosition, type Window } from "../window";
import { MenuItemBase, newMenu, type ItemKind } from "./base";
...
export class Submenu extends MenuItemBase {
	...
	static async new(opts: SubmenuOptions): Promise<Submenu> {
		return newMenu("Submenu", opts).then(
			([rid, id]) => new Submenu(rid, id),
		);
	}
	...
	async popup(
		...
		if (at) {
			...
			atValue[
				`${at instanceof PhysicalPosition ? "Physical" : "Logical"}`
			] = {
				x: at.x,
				y: at.y,
			};
