...
fn handle_file_associations(app:AppHandle, files:Vec<PathBuf>) {
	...
	// You can remove this block if you only want to know about the paths, but not
	// actually "use" them in the frontend.

	// This requires the `fs` tauri plugin and is required to make the plugin's
	// frontend work: use tauri_plugin_fs::FsExt;
