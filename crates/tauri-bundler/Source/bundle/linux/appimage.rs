...
pub fn bundle_project(settings:&Settings) -> crate::Result<Vec<PathBuf>> {
	...
	// Using create_dir_all for a single dir so we don't get errors if the path
	// already exists
	...
	for file in ["WebKitNetworkProcess", "WebKitWebProcess", "libwebkit2gtkinjectedbundle.so"] {
	...
	// Linuxdeploy logs everything into stderr so we have to ignore the output
	// ourselves here
