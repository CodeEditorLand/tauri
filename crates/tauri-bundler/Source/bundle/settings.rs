...
impl PackageType {
	...
	/// Possible values are "deb", "ios", "msi", "app", "rpm", "appimage",
	/// "dmg", "updater".
...
pub struct DebianSettings {
	...
	/// Maps the path on the debian package to the path of the file to include
...
pub struct RpmSettings {
	...
	/// Maps the path on the RPM package to the path of the file to include
	/// (relative to the current working directory).
	pub files:HashMap<PathBuf, PathBuf>,
	/// Path to a custom desktop file Handlebars template.
	///
...
pub struct MacOsSettings {
	...
	///
	/// Each string can either be the name of a framework (without the
	/// `.framework` extension, e.g. `"SDL2"`), in which case we will search
	/// for that framework in the standard install locations
	/// (`~/Library/Frameworks/`, `/Library/Frameworks/`, and
	/// `/Network/Library/Frameworks/`), or a path to a specific framework
	/// bundle (e.g. `./data/frameworks/SDL2.framework`).  Note that this
	/// setting just makes tauri-bundler copy the specified frameworks into the
	/// OS X app bundle (under `Foobar.app/Contents/Frameworks/`); you are
	/// still responsible for:
	///
	/// - arranging for the compiled binary to link against those frameworks
	///   (e.g. by emitting lines like `cargo:rustc-link-lib=framework=SDL2`
	///   from your `build.rs` script)
	///
	...
	/// Maps the path in the Contents directory in the app to the path of the
	/// file to include (relative to the current working directory).
	pub files:HashMap<PathBuf, PathBuf>,
	/// A version string indicating the minimum MacOS version that the bundled
	/// app supports (e.g. `"10.11"`). If you are using this config field, you
	/// may also want have your `build.rs` script emit
	/// `cargo:rustc-env=MACOSX_DEPLOYMENT_TARGET=10.11`.
	pub minimum_system_version:Option<String>,
	/// The exception domain to use on the macOS .app bundle.
	///
	/// This allows communication to the outside world e.g. a web server you're
	/// shipping.
	...
	///
	/// Settings this to `false` is useful when using an ad-hoc signature,
...
impl Default for WixLanguage {
	fn default() -> Self { Self(vec![("en-US".into(), Default::default())]) }
}
...
pub struct WixSettings {
	...
	/// derived from [`PackageSettings::version`] if this field is not set.
	///
	/// The first field is the major version and has a maximum value of 255. The
	/// second field is the minor version and has a maximum value of 255.
	...
	/// update as a different app and your users will have duplicate versions of
	/// your app.
	///
	...
	/// Tauri's CLI to generate and print this code for you by running `tauri
...
pub struct NsisSettings {
	...
	/// value is the path to a custom `.nsi` file that holds the translated text
	/// for tauri's custom messages.
	///
...
pub struct WindowsSettings {
	/// The file digest algorithm to use for creating file signatures. Required
	/// for code signing. SHA-256 is recommended.
	...
	#[deprecated = "This is used for the MSI installer and will be removed in 3.0.0, use \
	                `BundleSettings::icon` field and make sure a `.ico` icon exists instead."]
	...
	/// This command needs to have a `%1` in it which is just a placeholder for
	/// the binary path, which we will detect and replace before calling the
	/// command.
	///
	/// Example:
	/// ```text
...
pub struct BundleSettings {
	...
	/// The app's publisher. Defaults to the second element in the identifier
	/// string.
	///
	...
	/// when specified.
	///
	/// Maps each resource path to its target directory in the bundle resources
	/// directory.
	///
	/// Supports glob patterns.
	...
	///
	/// Note that each binary name should have the target platform's target
	/// triple appended, as well as `.exe` for Windows.
	/// For example, if you're bundling a sidecar called `sqlite3`, the bundler
	/// expects a binary named `sqlite3-x86_64-unknown-linux-gnu` on linux,
	...
	/// your external binary to also be universal, and named after the target
	/// triple, e.g. `sqlite3-universal-apple-darwin`. See
...
impl BundleBinary {
	...
	pub fn src_path(&self) -> Option<&String> { self.src_path.as_ref() }
}
...
pub struct Settings {
	...
	/// if `None`, tools are placed in the current user's platform-specific
	/// cache directory.
...
impl SettingsBuilder {
	...
	/// Sets the project output directory. It's used as current working
	/// directory.
	#[must_use]
	...
	/// Sets the log level for spawned commands. Defaults to
	/// [`log::Level::Error`].
	#[must_use]
	...
	///
	/// Bundle settings will be read from $TAURI_DIR/tauri.conf.json if it
	/// exists and fallback to Cargo.toml's [package.metadata.bundle].
...
impl Settings {
	...
	pub fn set_log_level(&mut self, level:log::Level) { self.log_level = level; }

	/// Returns the log level for spawned commands.
	pub fn log_level(&self) -> log::Level { self.log_level }

	/// Returns the directory where the bundle should be placed.
	pub fn project_out_directory(&self) -> &Path { &self.project_out_directory }

	/// Returns the target triple.
	pub fn target(&self) -> &str { &self.target }

	/// Returns the architecture for the binary being bundled (e.g. "arm", "x86"
	/// or "x86_64").
	...
	pub fn main_binary(&self) -> crate::Result<&BundleBinary> {
		self.binaries
			...
			.find(|bin| bin.main)
			.context(
				"failed to find main binary, make sure you have a `package > default-run` in the \
				 Cargo.toml file",
			)
			.map_err(Into::into)
	}
	...
	pub fn binary_path(&self, binary:&BundleBinary) -> PathBuf {
		let target_os = self.target().split('-').nth(2).unwrap_or(std::env::consts::OS);

		let path = self.project_out_directory.join(binary.name());

		if target_os == "windows" { path.with_extension("exe") } else { path }
	}

	/// Returns the list of binaries to bundle.
	pub fn binaries(&self) -> &Vec<BundleBinary> { &self.binaries }
	...
	pub fn product_name(&self) -> &str { &self.package.product_name }
	...
	pub fn publisher(&self) -> Option<&str> { self.bundle_settings.publisher.as_deref() }
	...
	pub fn resource_files(&self) -> ResourcePaths<'_> {
		match (&self.bundle_settings.resources, &self.bundle_settings.resources_map) {
	...
	pub fn version_string(&self) -> &str { &self.package.version }

	/// Returns the copyright text.
	pub fn copyright_string(&self) -> Option<&str> { self.bundle_settings.copyright.as_deref() }
	...
	pub fn license(&self) -> Option<String> { self.bundle_settings.license.clone() }

	/// Returns the bundle license file.
	pub fn license_file(&self) -> Option<PathBuf> { self.bundle_settings.license_file.clone() }

	/// Returns the package's homepage URL, defaulting to "" if not defined.
	pub fn homepage_url(&self) -> Option<&str> {
		self.bundle_settings.homepage.as_deref().or(self.package.homepage.as_deref())
	}

	/// Returns the app's category.
	pub fn app_category(&self) -> Option<AppCategory> { self.bundle_settings.category }
	...
	pub fn local_tools_directory(&self) -> Option<&Path> { self.local_tools_directory.as_deref() }

	/// Returns the debian settings.
	pub fn deb(&self) -> &DebianSettings { &self.bundle_settings.deb }

	/// Returns the appimage settings.
	pub fn appimage(&self) -> &AppImageSettings { &self.bundle_settings.appimage }

	/// Returns the RPM settings.
	pub fn rpm(&self) -> &RpmSettings { &self.bundle_settings.rpm }

	/// Returns the DMG settings.
	pub fn dmg(&self) -> &DmgSettings { &self.bundle_settings.dmg }

	/// Returns the MacOS settings.
	pub fn macos(&self) -> &MacOsSettings { &self.bundle_settings.macos }

	/// Returns the Windows settings.
	pub fn windows(&self) -> &WindowsSettings { &self.bundle_settings.windows }

	/// Returns the Updater settings.
	pub fn updater(&self) -> Option<&UpdaterSettings> { self.bundle_settings.updater.as_ref() }
}
