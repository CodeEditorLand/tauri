...
pub fn setup(
	...
	let bundle_identifier_source =
		config_.find_bundle_identifier_overwriter().unwrap_or_else(|| "tauri.conf.json".into());
	...
	if config_.identifier.chars().any(|ch| !(ch.is_alphanumeric() || ch == '-' || ch == '.')) {
		log::error!(
