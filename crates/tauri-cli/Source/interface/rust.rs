...
impl Interface for Rust {
	...
	fn new(config:&Config, target:Option<String>) -> crate::Result<Self> {
		...
		let target_ios = target
			.as_ref()
			.map_or(false, |target| target.ends_with("ios") || target.ends_with("ios-sim"));
...
fn build_ignore_matcher(dir:&Path) -> IgnoreMatcher {
		...
		.flatten()
	{
		...
		if path.file_name() == Some(OsStr::new(".taurignore")) {
			...
			for line in
				crate::dev::TAURI_CLI_BUILTIN_WATCHER_IGNORE_FILE.lines().map_while(Result::ok)
			{
...
fn shared_options(
	...
	if mobile {
		...
		if !all_features.contains(&"tauri/rustls-tls".into()) {
			features.get_or_insert(Vec::new()).push("tauri/native-tls".into());
		}
	}
}
				...
				for event in events {
					...
					if !ignore_matcher.is_ignore(&event_path, event_path.is_dir()) {
						...
						log::info!(
							...
							display_path(
								event_path.strip_prefix(frontend_path).unwrap_or(&event_path)
							)
						);
...
impl<'de> Deserialize<'de> for WindowsUpdateInstallMode {
	...
	where
		...
		match s.to_lowercase().as_str() {
			...
			_ => Err(serde::de::Error::custom(format!("unknown update install mode '{s}'"))),
		}
	}
}
...
impl AppSettings for RustAppSettings {
	...
	fn app_binary_path(&self, options:&Options) -> crate::Result<PathBuf> {
		...
		let out_dir = self.out_dir(options).context("failed to get project out directory")?;

		let ext = if self.target_triple.contains("windows") { "exe" } else { "" };
	...
	fn get_binaries(&self) -> crate::Result<Vec<BundleBinary>> {
		...
		if let Some(bins) = &self.cargo_settings.bin {
			...
			for bin in bins {
				let is_main =
					bin.name == self.cargo_package_settings.name || bin.name == default_run;

				binaries.push(BundleBinary::with_path(bin.name.clone(), is_main, bin.path.clone()))
			}
		}
		...
		if !binaries_paths.iter().any(|(_name, path)| path == Path::new("src/main.rs"))
			&& tauri_dir.join("src/main.rs").exists()
		{
			binaries_paths
				.push((self.cargo_package_settings.name.clone(), tauri_dir.join("src/main.rs")));
		}
		...
		match binaries.len() {
			0 => binaries.push(BundleBinary::new(self.cargo_package_settings.name.clone(), true)),
		...
		let package_settings = PackageSettings {
			...
			version:config.version.clone().unwrap_or_else(|| {
				cargo_package_settings
					...
					.resolve("version", || {
						ws_package_settings.as_ref().and_then(|p| p.version.clone()).ok_or_else(
							|| {
								anyhow::anyhow!(
					...
					description
						.resolve("description", || {
							ws_package_settings
								...
								.ok_or_else(|| {
									anyhow::anyhow!(
										"Couldn't inherit value for `description` from workspace"
									)
								})
						})
						.unwrap()
...
fn tauri_config_to_bundle_settings(
	...
	#[allow(unused_mut)]
	let mut resources = config.resources.unwrap_or(BundleResources::List(Vec::new()));
	#[allow(unused_mut)]
	...
	#[cfg(target_os = "linux")]
	{
			...
			|| enabled_features.contains(&"tauri/tray-icon".into())
		{
			...
			// conditionally setting it in case the user provided its own version for some
			// reason
			let path = PathBuf::from(path);
			if !appimage_files.contains_key(&path) {
				// manually construct target path, just in case the source path is something
				// unexpected
