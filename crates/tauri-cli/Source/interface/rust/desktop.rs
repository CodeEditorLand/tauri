...
impl DevProcess for DevChild {
	...
	fn try_wait(&self) -> std::io::Result<Option<ExitStatus>> { self.dev_child.try_wait() }

	fn wait(&self) -> std::io::Result<ExitStatus> { self.dev_child.wait() }
...
pub fn run_dev<F:Fn(Option<i32>, ExitReason) + Send + Sync + 'static>(
	...
	// TODO: remove this and DevChild (requires refactor for code shared between
	// mobile and desktop)
	let manually_killed_app = Arc::new(AtomicBool::default());

	Ok(DevChild { manually_killed_app, dev_child })
}
