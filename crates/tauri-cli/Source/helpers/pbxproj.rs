...
impl Pbxproj {
	...
	pub fn set_build_settings(&mut self, build_configuration_id:&str, key:&str, value:&str) {
		...
		if let Some(build_setting) =
			build_configuration.build_settings.iter_mut().find(|s| s.key == key)
		{
			...
			let Some(last_build_setting) = build_configuration.build_settings.last().cloned()
			else {
				return;
			};
...
mod tests {
	...
	fn modify() {
		...
		pbxproj.set_build_settings("DB_0E254D0FD84970B57F6410", "PRODUCT_NAME", "\"Tauri Test\"");
