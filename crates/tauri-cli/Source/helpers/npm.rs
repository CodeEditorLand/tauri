...
impl PackageManager {
	/// Detects package manager from the given directory, falls back to
	/// [`PackageManager::Npm`].
	pub fn from_project<P:AsRef<Path>>(path:P) -> Self {
		Self::all_from_project(path).first().copied().unwrap_or(Self::Npm)
	}
	...
	pub fn remove<P:AsRef<Path>>(
		...
		let status = self
			.cross_command()
			.arg(if *self == Self::Npm { "uninstall" } else { "remove" })
			.args(dependencies)
			.current_dir(frontend_dir)
			.status()
			.with_context(|| format!("failed to run {self}"))?;
