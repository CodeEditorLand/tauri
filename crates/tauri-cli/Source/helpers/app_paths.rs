	...
	builder
		...
		.max_depth(Some(
			std::env::var("TAURI_CLI_CONFIG_DEPTH")
				.map(|d| {
					d.parse().expect(
						"`TAURI_CLI_CONFIG_DEPTH` environment variable must be a positive integer",
					)
				})
				.unwrap_or(3),
		))
...
pub fn resolve() {
	TAURI_DIR
		.set(resolve_tauri_dir().unwrap_or_else(|| {
			let env_var_name =
				env_tauri_app_path().is_some().then(|| format!("`{ENV_TAURI_APP_PATH}`"));

			panic!(
				"Couldn't recognize the {} folder as a Tauri project. It must contain a `{}`, \
...
pub fn tauri_dir() -> &'static PathBuf {
	TAURI_DIR.get().expect("app paths not initialized, this is a Tauri CLI bug")
}
...
pub fn frontend_dir() -> &'static PathBuf {
	FRONTEND_DIR.get().expect("app paths not initialized, this is a Tauri CLI bug")
}
