	...
	call_for_targets_with_fallback(
		...
		|target:&Target| {
			...
			if !validated_lib {
				...
				let lib_path =
					config.app().target_dir(target.triple, profile).join(config.so_name());
...
fn adb_forward_port(
	...
	// if we could not detect any running device, let's wait a few seconds, it might
	// be booting up
	if devices.is_empty() {
		log::warn!(
			"ADB device list is empty, waiting a few seconds to see if there's any booting \
			 device..."
		);
	...
	let target_device = if let Some(target_device) = &cli_options.target_device {
		...
		anyhow::bail!(
			"Multiple Android devices are connected ({}), please disconnect devices you do not \
			 intend to use so Tauri can determine which to use",
			devices.iter().map(|d| d.name()).collect::<Vec<_>>().join(", ")
		);
	} else {
		// when building the app without running to a device, we might have an empty
		// devices list
		None
	};
		...
		if already_forwarded {
			...
			loop {
				run_adb_reverse(env, &target_device_serial_no, &forward, &forward).with_context(
					|| {
						format!(
							"failed to forward port with adb, is the {target_device_name} device \
							 connected?",
						)
					},
				)?;
