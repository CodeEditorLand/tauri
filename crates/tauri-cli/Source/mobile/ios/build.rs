	...
	call_for_targets_with_fallback(
		...
		|target:&Target| -> Result<()> {
			...
			if let Some(credentials) = &credentials {
				build_config =
					build_config.authentication_credentials(credentials.clone()).skip_codesign();
			}
			...
			target.archive(config, env, noise_level, profile, Some(app_version), archive_config)?;
			...
			if target.sdk == "iphonesimulator" {
				...
				// if we skipped code signing, we do not have the entitlements applied to our
				// exported IPA we must force sign the app binary with a dummy certificate
				// just to preserve the entitlements target.export() will sign it with an
				// actual certificate for us
				if skip_signing {
					...
					let certificate = tauri_macos_sign::certificate::generate_self_signed(
						tauri_macos_sign::certificate::SelfSignedCertificateRequest {
							...
							profile:
								tauri_macos_sign::certificate::CertificateProfile::AppleDistribution,
					...
					let self_signed_cert_keychain =
						tauri_macos_sign::Keychain::with_certificate_file(
							&cert_path,
							&password.into(),
						)?;
