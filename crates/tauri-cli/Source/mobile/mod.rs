...
use crate::{
	...
	helpers::{
		app_paths::tauri_dir,
		config::{Config as TauriConfig, ConfigHandle, reload as reload_config},
	},
...
impl Default for DevHost {
	fn default() -> Self {
		// on Windows we want to force using the public network address for the
		// development server because the adb port forwarding does not work well
		if cfg!(windows) { Self(Some(None)) } else { Self(None) }
	}
}
...
fn use_network_address_for_dev_url(
	...
	let ip = if let Some(url) = &mut dev_url {
		...
		if localhost {
			let ip = dev_options.host.unwrap_or_else(|| *local_ip_address(force_ip_prompt));
			log::info!(
			...
			*url = url::Url::parse(&format!(
			...
			if let Some(c) = &mut dev_options.config {
				...
				dev_options.config.replace(crate::ConfigValue(serde_json::json!({
				  "build": build
				})));
			}
	...
	let mut dev_url_config = DevUrlConfig { no_dev_server_wait:false };
	...
	match target {
		...
		Target::Ios => {
			let pbxproj_contents = read_to_string(
				project_dir.join(format!("{}.xcodeproj", app.name())).join("project.pbxproj"),
			)
