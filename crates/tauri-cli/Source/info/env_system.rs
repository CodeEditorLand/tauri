...
fn build_tools_version() -> crate::Result<Vec<String>> {
	...
	// Check if there are Visual Studio installations that have the "MSVC - C++
	// Buildtools" and "Windows SDK" components. Both the Windows 10 and Windows 11
	// SDKs work so we need to query it twice.
...
fn webkit2gtk_ver() -> Option<String> { pkg_conf_version("webkit2gtk-4.1") }
#[cfg(any(
	target_os = "linux",
	target_os = "dragonfly",
	target_os = "freebsd",
...
fn rsvg2_ver() -> Option<String> { pkg_conf_version("librsvg-2.0") }
	...
	vec![
		...
		SectionItem::new().action(|| {
			...
			if build_tools.is_empty() {
				(
					format!(
						"Couldn't detect any Visual Studio or VS Build Tools instance with MSVC \
						 and SDK components. Download from {}",
		...
		SectionItem::new().action(|| {
			if is_xcode_command_line_tools_installed() {
				("Xcode Command Line Tools: installed".into(), Status::Success)
			} else {
				(
					format!(
