...
pub fn command(options:Options) -> Result<()> {
	...
	if acl_manifests_path.exists() {
		...
		for (key, manifest) in acl {
			if options.plugin.as_ref().map(|p| p != &key).unwrap_or_default() {
				continue;
			}
			...
			for set in manifest.permission_sets.values() {
				if options.filter.as_ref().map(|f| set.identifier.contains(f)).unwrap_or(true) {
					permissions.push(format!(
						"{prefix}{}\n{}\nPermissions: {}",
						set.identifier.cyan(),
						set.description,
			...
			for permission in manifest.permissions.into_values() {
					...
					.unwrap_or(true)
				{
					permissions.push(format!(
						...
						permission.description.map(|d| format!("\n{d}")).unwrap_or_default(),
