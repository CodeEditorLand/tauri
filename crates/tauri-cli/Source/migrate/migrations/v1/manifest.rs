...
fn migrate_manifest(manifest:&mut DocumentMut) -> Result<()> {
	...
	if let Some(lib) = manifest.as_table_mut().get_mut("lib").and_then(|l| l.as_table_mut()) {
...
fn dependency_version() -> String {
	...
	if pre.is_empty() {
		...
		format!("{}.0.0-{}", env!("CARGO_PKG_VERSION_MAJOR"), pre.split('.').next().unwrap())
	}
}
...
mod tests {
	...
	fn migrate_deps<F:FnOnce(&[&str]) -> String>(get_toml:F) {
		...
		let tauri = dependencies.get("tauri").expect("missing tauri dependency in manifest");
