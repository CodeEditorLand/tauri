...
pub fn generate_gradle_files(
	...
	let app_build_gradle_path =
		project_dir.join("app").join("tauri.build.gradle.kts");
	let app_tauri_properties_path =
		project_dir.join("app").join("tauri.properties");

	let mut gradle_settings = "// THIS IS AN AUTOGENERATED FILE. DO NOT EDIT \
	                           THIS FILE DIRECTLY.\n"
		.to_string();
	let mut app_build_gradle = "// THIS IS AN AUTOGENERATED FILE. DO NOT EDIT \
	                            THIS FILE DIRECTLY.
...
dependencies {"
	...
	for (env, value) in std::env::vars_os() {
		...
		if env.starts_with("DEP_") && env.ends_with("_ANDROID_LIBRARY_PATH") {
			let name_len =
				env.len() - "DEP_".len() - "_ANDROID_LIBRARY_PATH".len();
			...
			app_build_gradle.push_str(&format!(
				r#"  implementation(project(":{plugin_name}"))"#
			));
		}
	}
	...
	if let Some(version) = config.version.as_ref() {
		app_tauri_properties
			.push(format!("tauri.android.versionName={version}"));
		if let Some(version_code) = config.bundle.android.version_code.as_ref()
		{
			app_tauri_properties
				.push(format!("tauri.android.versionCode={version_code}"));
		} else if let Ok(version) = Version::parse(version) {
			let mut version_code =
			...
			if version_code == 0 {
				return Err(anyhow::anyhow!(
					"You must change the `version` in `tauri.conf.json`. The \
					 default value `0.0.0` is not allowed for Android package \
					 and must be at least `0.0.1`."
				));
			} else if version_code > 2100000000 {
				return Err(anyhow::anyhow!(
					"Invalid version code {}. Version code must be between 1 \
					 and 2100000000. You must change the `version` in \
					 `tauri.conf.json`.",
					version_code
				));
			}
