...
pub fn get_config(path:&Path) -> Result<(Config, PathBuf), CodegenConfigError> {
	...
	// in the future we may want to find a way to not need the TAURI_CONFIG env var
	// so that it is impossible for the content of two separate configs to get
	...
	// Set working directory to where `tauri.config.json` is, so that relative paths
