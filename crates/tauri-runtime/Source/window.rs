...
impl<T:UserEvent, R:Runtime<T>> PendingWindow<T, R> {
	...
	pub fn new(
		...
		if !is_label_valid(&label) {
			...
			Ok(Self { window_builder, label, webview:None })
		}
	}
...
impl<T:UserEvent, R:Runtime<T>> Clone for DetachedWindowWebview<T, R> {
	fn clone(&self) -> Self {
		Self { webview:self.webview.clone(), use_https_scheme:self.use_https_scheme }
	}
}
