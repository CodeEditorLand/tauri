...
pub trait Runtime<T:UserEvent>: Debug + Sized + 'static {
	...
	/// Runs an iteration of the runtime event loop and returns control flow to
	/// the caller.
	#[cfg(desktop)]
...
pub trait WebviewDispatch<T:UserEvent>: Debug + Clone + Send + Sync + Sized + 'static {
	...
	/// Returns the position of the top-left hand corner of the webviews's
	/// client area relative to the top-left hand corner of the window.
...
pub trait WindowDispatch<T:UserEvent>: Debug + Clone + Send + Sync + Sized + 'static {
	...
	///
	/// ## Platform-specific
	/// - **Windows:** Unsupported, use [`WindowDispatch::set_overlay_icon`]
	///   instead.
	/// - **Android:** Unsupported.
	/// - **iOS:** iOS expects i32, if the value is larger than i32::MAX, it
	///   will be clamped to i32::MAX.
	fn set_badge_count(&self, count:Option<i64>, desktop_filename:Option<String>) -> Result<()>;

	/// Sets the badge count on the taskbar **macOS only**. Using `None` will
	/// remove the badge
