...
//!
//! Create macros for `tauri::Context`, invoke handler and commands leveraging
//! the `tauri-codegen` crate.
...
/// Accepts a list of command functions. Creates a handler that allows commands
...
/// Reads a Tauri config file and generates a `::tauri::Context` based on the
/// content.
///
/// # Stability
/// The output of this macro is managed internally by Tauri,
...
/// specific feature.
///
/// e.g. To default the runtime generic to type `crate::Wry` when the `wry`
/// feature is enabled, the syntax would look like
/// `#[default_runtime(crate::Wry, wry)`. This is **always** set for the last
...
/// after matching against `kind` and retrieving it from `resources_table` using
/// `rid`.
///
