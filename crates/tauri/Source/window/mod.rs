...
```"####
	...
	fn build_internal(
		...
		#[cfg(desktop)]
		let handler = app_manager.menu.prepare_window_menu_creation_handler(
			window_menu.as_ref(),
			self.window_builder.get_theme(),
		);
		#[cfg(not(desktop))]
...
```
  "####
	...
	pub(crate) fn has_app_wide_menu(&self) -> bool {
		self.menu_lock().as_ref().map(|m| m.is_app_wide).unwrap_or(false)
	}
	...
	pub fn set_menu(&self, menu:Menu<R>) -> crate::Result<Option<Menu<R>>> {
		...
		self.run_on_main_thread(move || {
			...
			if let (Ok(gtk_window), Ok(gtk_box)) = (window.gtk_window(), window.default_vbox()) {
				let _ = menu_.inner().init_for_gtk_window(&gtk_window, Some(&gtk_box));
			}
		})?;
...
impl<R:Runtime> Window<R> {
	...
	pub fn ns_window(&self) -> crate::Result<*mut std::ffi::c_void> {
		self.window.dispatcher.window_handle().map_err(Into::into).and_then(|handle| {
...
impl<R:Runtime> Window<R> {
	...
	pub fn set_enabled(&self, enabled:bool) -> crate::Result<()> {
		self.window.dispatcher.set_enabled(enabled).map_err(Into::into)
	}
...
```
  "####
	...
	pub fn set_background_color(&self, color:Option<Color>) -> crate::Result<()> {
		self.window.dispatcher.set_background_color(color).map_err(Into::into)
	}
	...
	/// Sets the overlay icon on the taskbar **Windows only**. Using `None` to
	...
	pub fn set_badge_label(&self, label:Option<String>) -> crate::Result<()> {
		self.window.dispatcher.set_badge_label(label).map_err(Into::into)
	}
	...
	pub fn set_theme(&self, theme:Option<Theme>) -> crate::Result<()> {
		self.window.dispatcher.set_theme(theme).map_err(Into::<crate::Error>::into)?;
		#[cfg(windows)]
...
/// Progress bar state.
#[cfg(desktop)]
#[cfg_attr(docsrs, doc(cfg(any(target_os = "macos", target_os = "linux", windows))))]
