...
impl<R: Runtime> WebviewManager<R> {
	...
	fn prepare_pending_webview<M: Manager<R>>(
		...
		let plugin_init_scripts =
			app_manager.plugins.lock().expect("poisoned plugin store").initialization_script();
	...
	fn initialization_script(
		...
		let bundle_script =
			if with_global_tauri { include_str!("../../scripts/bundle.global.js") } else { "" };
	...
	pub fn prepare_webview<M: Manager<R>>(
		...
		pending.navigation_handler = Some(Box::new(move |url| {
			...
			if let Some(w) = webview {
				app_manager.plugins.lock().expect("poisoned plugin store").on_navigation(&w, url)
			} else {
				true
			}
		}));
