...
impl<R:Runtime> AppManager<R> {
	...
	pub fn get_asset(
		...
		let asset_response = assets
			...
			.or_else(|| {
				log::debug!("Asset `{}` not found; fallback to {}/index.html", path, path);
	...
	pub fn emit<S:Serialize + Clone>(&self, event:&str, payload:S) -> crate::Result<()> {
		...
		let webviews = self.webview.webviews_lock().values().cloned().collect::<Vec<_>>();
	...
	where
		...
		match target {
			...
			EventTarget::Window { label: target_label } => {
				self.emit_filter(event, payload, |t| {
					match t {
						EventTarget::AnyLabel { label } | EventTarget::Window { label } => {
							label == &target_label
						},
						_ => false,
					}
			...
			EventTarget::Webview { label: target_label } => {
				self.emit_filter(event, payload, |t| {
					match t {
						EventTarget::AnyLabel { label } | EventTarget::Webview { label } => {
							label == &target_label
						},
						_ => false,
					}
			...
			EventTarget::WebviewWindow { label: target_label } => {
				self.emit_filter(event, payload, |t| {
					match t {
...
mod test {
	...
	fn setup_events(setup_any:bool) -> EventSetup {
			...
			($type:ident, $id:ident, $any_id:ident) => {
				...
				if setup_any {
					...
					$type.listen_any(TEST_EVENT_NAME, move |evt| {
						tx_.send(($any_id, serde_json::from_str::<String>(evt.payload()).unwrap()))
							.unwrap();
					});
				}
			};
		...
		setup_listener!(webview_window, WEBVIEW_WINDOW_LISTEN_ID, WEBVIEW_WINDOW_LISTEN_ANY_ID);
	...
	#[test]
	fn emit() {
		let EventSetup { app, window, webview, webview_window, tx: _, rx } = setup_events(true);
	...
	#[test]
	fn emit_to() {
		let EventSetup { app, window, webview, webview_window, tx, rx } = setup_events(false);
	...
	fn run_emit_to_test<M:Manager<MockRuntime> + Emitter<MockRuntime>>(
		...
		m.emit_to(other_webview.label(), TEST_EVENT_NAME, payload).unwrap();
