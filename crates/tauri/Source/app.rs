...
impl ExitRequestApi {
	...
	pub fn prevent_exit(&self) { self.0.send(ExitRequestedEventAction::Prevent).unwrap(); }
}
...
impl From<RuntimeWindowEvent> for WindowEvent {
	fn from(event:RuntimeWindowEvent) -> Self {
		match event {
			...
			RuntimeWindowEvent::ScaleFactorChanged { scale_factor, new_inner_size } => {
...
pub enum RunEvent {
	...
	Reopen {
		/// Indicates whether the NSApplication object found any visible windows
		/// in your application.
...
impl<R:Runtime> AssetResolver<R> {
	...
	///
	/// By default it tries to infer your application's URL scheme in production
	/// by checking if all webviews were configured with
	/// [`crate::webview::WebviewBuilder::use_https_scheme`] or `tauri.conf.json
	/// > app > windows > useHttpsScheme`. If you are resolving an asset for a
	/// webview with a more dynamic configuration, see
	/// [`AssetResolver::get_for_scheme`].
	///
	...
	pub fn get(&self, path:String) -> Option<Asset> {
		let use_https_scheme =
			self.manager.webviews().values().all(|webview| webview.use_https_scheme());

		self.get_for_scheme(path, use_https_scheme)
	}

	///  Same as [AssetResolver::get] but resolves the custom protocol scheme
	...
	pub fn get_for_scheme(&self, path:String, use_https_scheme:bool) -> Option<Asset> {
		#[cfg(dev)]
		{
			...
			// we only fallback to reading from distDir directly if we're using an external
			// URL (which is likely)
			if let (Some(_), Some(crate::utils::config::FrontendDist::Directory(dist_path))) =
				(&self.manager.config().build.dev_url, &self.manager.config().build.frontend_dist)
			{
...
impl<R:Runtime> Clone for AppHandle<R> {
	fn clone(&self) -> Self {
		Self { runtime_handle:self.runtime_handle.clone(), manager:self.manager.clone() }
	}
}
...
impl<R:Runtime> AppHandle<R> {
	...
	pub fn set_activation_policy(&self, activation_policy:ActivationPolicy) -> crate::Result<()> {
		self.runtime_handle.set_activation_policy(activation_policy).map_err(Into::into)
	}
}

impl<R:Runtime> Manager<R> for AppHandle<R> {
	fn resources_table(&self) -> MutexGuard<'_, ResourceTable> { self.manager.resources_table() }
}
...
macro_rules! shared_app_impl {
	($app:ty) => {
		impl<R:Runtime> $app {
			...
			/// disappear if it wasn't cloned somewhere else or referenced by JS.
			#[cfg(all(desktop, feature = "tray-icon"))]
			#[cfg_attr(docsrs, doc(cfg(all(desktop, feature = "tray-icon"))))]
			...
			pub fn package_info(&self) -> &PackageInfo { self.manager.package_info() }
			...
			#[cfg(desktop)]
			pub fn menu(&self) -> Option<Menu<R>> { self.manager.menu.menu_lock().clone() }
			...
			pub fn remove_menu(&self) -> crate::Result<Option<Menu<R>>> {
				...
				self.manager.remove_menu_from_stash_by_id(prev_menu.as_ref().map(|m| m.id()));
			...
			/// **You should always exit the tauri app immediately after this function
			/// returns and not use any tauri-related APIs.**
			...
			/// # Security
			///
			/// DO NOT expose this key to third party scripts as might grant access to
			/// the backend from external URLs and iframes.
			pub fn invoke_key(&self) -> &str { self.manager.invoke_key() }
		}
...
pub struct Builder<R:Runtime> {
	...
	/// The script that initializes the `window.__TAURI_INTERNALS__.postMessage`
	/// function.
...
impl<R:Runtime> Builder<R> {
	...
	/// Registers a channel interceptor that can overwrite the default channel
	/// implementation.
	///
	/// If the event has been consumed, it must return `true`.
	///
	/// The channel automatically orders the messages, so the third closure
	/// argument represents the message number. The payload expected by the
	/// channel receiver is in the form of `{ id: usize, message: T }`.
	...
	/// "#;
	///
	/// tauri::Builder::default().append_invoke_initialization_script(custom_script);
	/// ```
...
```
  "####
	...
	/// }
	///
	/// tauri::Builder::default().plugin(plugin::init());
	/// ```
	#[must_use]
	...
	///
	/// #[tauri::command]
	/// fn int_command(state:State<MyInt>) -> String { format!("The stateful int is: {}", state.0) }
	///
	/// #[tauri::command]
	...
	/// use tauri::menu::*;
	///
	/// tauri::Builder::default().on_menu_event(|app, event| {
	/// 	if event.id() == "quit" {
	/// 		app.exit(0);
	/// 	}
	/// });
	...
	/// use tauri::Manager;
	///
	/// tauri::Builder::default().on_tray_icon_event(|app, event| {
	...
	/// # Examples
	/// ```
	/// tauri::Builder::default().register_uri_scheme_protocol("app-files", |_ctx, request| {
	/// 	// skip leading `/`
	...
	#[cfg_attr(feature = "tracing", tracing::instrument(name = "app::build", skip_all))]
	pub fn build(mut self, context:Context<R>) -> crate::Result<App<R>> {
		#[cfg(target_os = "macos")]
		if self.menu.is_none() && self.enable_macos_default_menu {
			self.menu = Some(Box::new(|app_handle| crate::menu::Menu::default(app_handle)));
		}
		...
		#[cfg(windows)]
		{
				...
				&app.manager.config().bundle.windows.webview_install_mode
			{
				if let Ok(resource_dir) = app.path().resource_dir() {
					...
					eprintln!(
						"failed to resolve resource directory; fallback to the installed Webview2 \
						 runtime."
					);
				}
			}
...
fn on_event_loop_event<R:Runtime>(
	...
	if let RuntimeRunEvent::WindowEvent { label, event: RuntimeWindowEvent::Destroyed } = &event {
	...
	let event = match event {
		...
		RuntimeRunEvent::UserEvent(t) => {
			match t {
				...
				EventLoopMessage::TrayIconEvent(ref e) => {
					for listener in &*app_handle.manager.tray.global_event_listeners.lock().unwrap()
					{
						listener(app_handle, e.clone());
					}
		...
		#[cfg(target_os = "macos")]
		RuntimeRunEvent::Reopen { has_visible_windows } => RunEvent::Reopen { has_visible_windows },
		_ => unimplemented!(),
	};
