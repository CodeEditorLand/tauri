...
pub(crate) type GlobalMenuEventListener<T> =
	Box<dyn Fn(&T, crate::menu::MenuEvent) + Send + Sync>;
#[cfg(all(desktop, feature = "tray-icon"))]
...
pub type OnPageLoad<R> =
	dyn Fn(&Webview<R>, &PageLoadPayload<'_>) + Send + Sync + 'static;

/// The exit code on [`RunEvent::ExitRequested`] when
/// [`AppHandle#method.restart`] is called.
...
impl CloseRequestApi {
	...
	pub fn prevent_close(&self) { self.0.send(true).unwrap(); }
}
...
pub enum WindowEvent {
	...
	/// The system window theme has changed. Only delivered if the window
...
impl From<RuntimeWindowEvent> for WindowEvent {
	fn from(event:RuntimeWindowEvent) -> Self {
		match event {
			...
			RuntimeWindowEvent::CloseRequested { signal_tx } => {
				Self::CloseRequested { api:CloseRequestApi(signal_tx) }
			},
			...
			RuntimeWindowEvent::ScaleFactorChanged {
				scale_factor,
				new_inner_size,
			} => Self::ScaleFactorChanged { scale_factor, new_inner_size },
			RuntimeWindowEvent::DragDrop(event) => Self::DragDrop(event),
			RuntimeWindowEvent::ThemeChanged(theme) => {
...
pub enum RunEvent {
	...
	ExitRequested {
		...
		/// [`Option::Some`] when requested programmatically via
		/// [`AppHandle#method.exit`] and [`AppHandle#method.restart`].
		code:Option<i32>,
		/// Event API
		api:ExitRequestApi,
	},
	...
	/// An event from a menu item, could be on the window menu bar, application
	/// menu bar (on macOS) or tray icon menu.
	#[cfg(desktop)]
	...
	/// Emitted when the NSApplicationDelegate's applicationShouldHandleReopen
	/// gets called
	#[non_exhaustive]
	#[cfg(target_os = "macos")]
	#[cfg_attr(docsrs, doc(cfg(target_os = "macos")))]
	Reopen {
		/// Indicates whether the NSApplication object found any visible
...
impl<R:Runtime> AssetResolver<R> {
	...
	///
	/// Fallbacks to reading the asset from the [distDir] folder so the behavior
	/// is consistent in development. Note that the dist directory must exist
	/// so you might need to build your frontend assets first.
	pub fn get(&self, path:String) -> Option<Asset> {
		#[cfg(dev)]
		{
			// on dev if the devPath is a path to a directory we have the
			// embedded assets so we can use get_asset() directly
			// we only fallback to reading from distDir directly if we're using
			// an external URL (which is likely)
			if let (
				Some(_),
				Some(crate::utils::config::FrontendDist::Directory(dist_path)),
			) = (
				...
				return std::fs::read(asset_path).ok().map(|bytes| {
					let mime_type =
						crate::utils::mime_type::MimeType::parse(&bytes, &path);
	...
	pub fn iter(&self) -> Box<AssetsIter<'_>> { self.manager.assets.iter() }
}
...
impl AppHandle<crate::Wry> {
	/// Create a new tao window using a callback. The event loop must be running
	/// at this point.
	pub fn create_tao_window<
		F:FnOnce() -> (String, tauri_runtime_wry::TaoWindowBuilder)
			+ Send
			+ 'static,
	>(
		&self,
...
impl<'de, R:Runtime> CommandArg<'de, R> for AppHandle<R> {
	...
	) -> std::result::Result<Self, InvokeError> {
...
impl<R:Runtime> AppHandle<R> {
	...
	/// This function can be used to register a plugin that is loaded
	/// dynamically e.g. after login. For plugins that are created when the app
	/// is started, prefer [`Builder::plugin`].
	///
	...
	/// tauri::Builder::default().plugin(plugin).setup(move |app| {
	/// 	let handle = app.handle().clone();
	/// 	std::thread::spawn(move || {
	...
	/// Exits the app by triggering [`RunEvent::ExitRequested`] and
	/// [`RunEvent::Exit`].
	...
	/// Restarts the app by triggering [`RunEvent::ExitRequested`] with code
	/// [`RESTART_EXIT_CODE`] and [`RunEvent::Exit`]..
	pub fn restart(&self) -> ! {
...
impl<R:Runtime> ManagerBase<R> for AppHandle<R> {
	...
	fn managed_app_handle(&self) -> &AppHandle<R> { self }
}
...
impl<R:Runtime> ManagerBase<R> for App<R> {
	...
	fn managed_app_handle(&self) -> &AppHandle<R> { self.handle() }
}

/// APIs specific to the wry runtime.
#[cfg(feature = "wry")]
impl App<crate::Wry> {
	/// Adds a [`tauri_runtime_wry::Plugin`] using its
	/// [`tauri_runtime_wry::PluginBuilder`].
	///
	/// # Stability
	///
	/// This API is unstable.
	pub fn wry_plugin<
		P:tauri_runtime_wry::PluginBuilder<EventLoopMessage> + Send + 'static,
	>(
		&mut self,
		plugin:P,
	) where
	...
	($app:ty) => {
		impl<R:Runtime> $app {
			...
			pub fn on_menu_event<
				F:Fn(&AppHandle<R>, MenuEvent) + Send + Sync + 'static,
			>(
				&self,
				handler:F,
			) {
			...
			/// Removes a tray icon using the provided id from tauri's internal
			/// state and returns it.
			///
			/// Note that dropping the returned icon, may cause the tray icon to
			/// disappear if it wasn't cloned somewhere else or referenced by
			/// JS.
			#[cfg(all(desktop, feature = "tray-icon"))]
			#[cfg_attr(docsrs, doc(cfg(all(desktop, feature = "tray-icon"))))]
			pub fn remove_tray_by_id<'a, I>(
				&self,
				id:&'a I,
			) -> Option<TrayIcon<R>>
			where
			...
			/// file.
			pub fn config(&self) -> &Config { self.manager.config() }
			...
			pub fn primary_monitor(&self) -> crate::Result<Option<Monitor>> {
				Ok(match self.runtime() {
					RuntimeOrDispatch::Runtime(h) => {
						h.primary_monitor().map(Into::into)
					},
			...
			/// Get the cursor position relative to the top-left hand corner of
			/// the desktop.
			///
			/// Note that the top-left hand corner of the desktop is not
			/// necessarily the same as the screen. If the user uses a desktop
			/// with multiple monitors, the top-left hand corner of the desktop
			/// is the top-left hand corner of the main monitor on Windows and
			/// macOS or the top-left of the leftmost monitor on X11.
			///
			/// The coordinates can be negative if the top-left hand corner of
			/// the window is outside of the visible screen region.
			...
			pub fn set_theme(&self, theme:Option<Theme>) {
				...
				for window in self.manager.windows().values() {
					if let (Some(menu), Ok(hwnd)) =
						(window.menu(), window.hwnd())
					{
			...
			pub fn set_menu(
				...
				// set it on all windows that don't have one or previously had
				// the app-wide menu
				#[cfg(not(target_os = "macos"))]
				{
					for window in self.manager.windows().values() {
						...
						if has_app_wide_menu {
							...
							window.menu_lock().replace(
								crate::window::WindowMenu {
									is_app_wide:true,
									menu:menu.clone(),
								},
							);
			...
			///
			/// If a window was not created with an explicit menu or had one set
			/// explicitly, this will remove the menu from it.
			#[cfg(desktop)]
			pub fn remove_menu(&self) -> crate::Result<Option<Menu<R>>> {
				...
				self.manager.remove_menu_from_stash_by_id(
					prev_menu.as_ref().map(|m| m.id()),
				);
			...
			///
			/// If a window was not created with an explicit menu or had one set
			/// explicitly, this will hide the menu from it.
			#[cfg(desktop)]
			pub fn hide_menu(&self) -> crate::Result<()> {
				#[cfg(not(target_os = "macos"))]
				{
					let is_app_menu_set =
						self.manager.menu.menu_lock().is_some();
					if is_app_menu_set {
						for window in self.manager.windows().values() {
			...
			///
			/// If a window was not created with an explicit menu or had one set
			/// explicitly, this will show the menu for it.
			#[cfg(desktop)]
			...
			/// **You should always exit the tauri app immediately after this
			/// function returns and not use any tauri-related APIs.**
		...
		impl<R:Runtime> Emitter<R> for $app {
			...
			/// Emits an event to all [targets](EventTarget) matching the given
			/// target.
			///
			/// # Examples
			/// ```
			...
			/// Emits an event to all [targets](EventTarget) based on the given
			/// filter.
			///
			/// # Examples
			/// ```
			...
			/// 		app.emit_filter("download-progress", i, |t| {
			/// 			match t {
...
impl<R:Runtime> App<R> {
	...
	pub fn handle(&self) -> &AppHandle<R> { &self.handle }
	...
	///
	/// Since the DeviceEvent capture can lead to high CPU usage for unfocused
	/// windows, [`tao`] will ignore them by default for unfocused windows on
	/// Windows. This method allows changing the filter to explicitly capture
	/// them again.
	///
	/// ## Platform-specific
	///
	...
	/// });
	/// ```
	pub fn run<F:FnMut(&AppHandle<R>, RunEvent) + 'static>(
		mut self,
		mut callback:F,
	) {
		...
		self.runtime.take().unwrap().run(move |event| {
			match event {
				...
				_ => {
					let event =
						on_event_loop_event(&app_handle, event, &manager);
					callback(&app_handle, event);
				},
			}
		});
	...
	/// The cleanup calls [`App::cleanup_before_exit`] so you may want to call
	/// that function before exiting the application.
	///
	/// # Examples
	/// ```no_run
	/// use tauri::Manager;
	...
	/// ```
	#[cfg(desktop)]
	pub fn run_iteration<F:FnMut(&AppHandle<R>, RunEvent) + 'static>(
		&mut self,
		mut callback:F,
	) {
		let manager = self.manager.clone();
...
pub struct Builder<R:Runtime> {
	...
	/// The script that initializes the
	...
	#[cfg(desktop)]
	menu:
		Option<Box<dyn FnOnce(&AppHandle<R>) -> crate::Result<Menu<R>> + Send>>,
	...
	pub fn new() -> Self {
		...
		Self {
			...
			invoke_initialization_script:InvokeInitializationScript {
				process_ipc_message_fn:
					crate::manager::webview::PROCESS_IPC_MESSAGE_FN,
...
impl<R:Runtime> Builder<R> {
	/// Builds a new Tauri application running on any thread, bypassing the main
	/// thread requirement.
	///
	/// ## Platform-specific
	///
	...
	/// ```
	/// #[tauri::command]
	/// fn command_1() -> String { return "hello world".to_string(); }
	...
	/// `window.__TAURI_INTERNALS__.postMessage`. That function must take the
	/// `(message: object, options: object)` arguments and send it to the
	/// backend.
	///
	/// Additionally, the script must include a `__INVOKE_KEY__` token that is
	/// replaced with a value that must be sent with the IPC payload
	/// to check the integrity of the message by the
	/// [`crate::WebviewWindow::on_message`] API, e.g.
	///
	/// ```js
	...
	///
	/// Allow to append custom initialization script instend of replacing entire
	/// invoke system.
	///
	/// # Examples
	///
...
```
  "####
	...
	/// Adds a Tauri application plugin.
	///
	/// A plugin is created using the [`crate::plugin::Builder`] struct.Check
	...
	/// Additionally, [`state`](crate::Manager#method.state) can be used to
	/// retrieve the value manually.
	///
	/// # Panics
	///
	...
	///
	/// tauri::Builder::default().menu(|handle| {
	/// 	Menu::with_items(
	/// 		handle,
	/// 		&[&Submenu::with_items(
	/// 			handle,
	...
	#[cfg(desktop)]
	pub fn menu<
		F:FnOnce(&AppHandle<R>) -> crate::Result<Menu<R>> + Send + 'static,
	>(
		mut self,
		f:F,
	) -> Self {
	...
	/// # Examples
	/// ```
	/// tauri::Builder::default().enable_macos_default_menu(false); 
	/// ```
	#[must_use]
	...
	pub fn on_window_event<
		F:Fn(&Window<R>, &WindowEvent) + Send + Sync + 'static,
	>(
		mut self,
		handler:F,
	) -> Self {
	...
	/// # Examples
	/// ```
	/// tauri::Builder::default().on_webview_event(|window, event| {
	/// 	match event {
	...
	/// ```
	#[must_use]
	pub fn on_webview_event<
		F:Fn(&Webview<R>, &WebviewEvent) + Send + Sync + 'static,
	>(
		mut self,
		handler:F,
	) -> Self {
	...
	/// # Examples
	/// ```
	/// tauri::Builder::default().register_uri_scheme_protocol(
	/// 	"app-files",
	/// 	|_ctx, request| {
	/// 		// skip leading `/`
	...
	/// Similar to [`Self::register_uri_scheme_protocol`] but with an
	/// asynchronous responder that allows you to process the request in a
	/// separate thread and respond asynchronously.
	///
	/// # Arguments
	///
	/// * `uri_scheme` The URI scheme to register, such as `example`.
	...
	/// # Examples
	/// ```
	/// tauri::Builder::default().register_asynchronous_uri_scheme_protocol(
	/// 	"app-files",
	...
	/// 				responder
	/// 					.respond(http::Response::builder().body(data).unwrap());
	/// 			} else {
	/// 				responder.respond(
	/// 					http::Response::builder()
	/// 						.status(http::StatusCode::BAD_REQUEST)
	...
	pub fn build(mut self, context:Context<R>) -> crate::Result<App<R>> {
		...
		#[cfg(desktop)]
		{
			...
			muda::MenuEvent::set_event_handler(Some(
				move |e:muda::MenuEvent| {
					let _ =
						proxy.send_event(EventLoopMessage::MenuEvent(e.into()));
				},
			));
				...
				tray_icon::TrayIconEvent::set_event_handler(Some(
					move |e:tray_icon::TrayIconEvent| {
						let _ = proxy.send_event(
							EventLoopMessage::TrayIconEvent(e.into()),
						);
					},
				));
			}
		...
		let mut app = App {
			...
			handle:AppHandle { runtime_handle, manager },
			ran_setup:false,
		};
		...
		#[cfg(windows)]
		{
			if let crate::utils::config::WebviewInstallMode::FixedRuntime {
				path,
			} = &app.manager.config().bundle.windows.webview_install_mode
			{
				if let Ok(resource_dir) = app.path().resource_dir() {
					...
					eprintln!(
						"failed to resolve resource directory; fallback to \
...
pub(crate) type UriSchemeResponderFn =
...
impl UriSchemeResponder {
	...
	pub fn respond<T:Into<Cow<'static, [u8]>>>(
		self,
...
impl<'a, R:Runtime> UriSchemeContext<'a, R> {
	...
	pub fn app_handle(&self) -> &'a AppHandle<R> { self.app_handle }

	/// Get the webview label that made the uri scheme request.
	pub fn webview_label(&self) -> &'a str { self.webview_label }
}
...
fn on_event_loop_event<R:Runtime>(
	...
	let event = match event {
		...
		RuntimeRunEvent::ExitRequested { code, tx } => {
			RunEvent::ExitRequested { code, api:ExitRequestApi(tx) }
		},
		RuntimeRunEvent::WindowEvent { label, event } => {
			RunEvent::WindowEvent { label, event:event.into() }
		},
		RuntimeRunEvent::WebviewEvent { label, event } => {
			RunEvent::WebviewEvent { label, event:event.into() }
		},
		RuntimeRunEvent::Ready => {
		...
		#[cfg(target_os = "macos")]
		RuntimeRunEvent::Reopen { has_visible_windows } => {
