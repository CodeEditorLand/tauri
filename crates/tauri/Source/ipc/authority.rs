...
impl RuntimeAuthority {
	...
	pub fn add_capability(&mut self, capability:impl RuntimeCapability) -> crate::Result<()> {
		...
		let resolved =
			Resolved::resolve(&self.acl, capabilities, tauri_utils::platform::Target::current())
				.unwrap();
		...
		for (cmd_key, resolved_cmds) in resolved.allowed_commands {
			...
			for resolved_cmd in &resolved_cmds {
				if let Some(scope_id) = resolved_cmd.scope_id {
					...
					self.scope_manager.command_cache.insert(scope_id, StateManager::new());
				}
			}
...
impl<'a, R:Runtime, T:ScopeObject> CommandArg<'a, R> for GlobalScope<T> {
	...
	fn from_command(command:CommandItem<'a, R>) -> Result<Self, InvokeError> {
		GlobalScope::resolve(&command.message.webview, command.plugin.unwrap_or(APP_ACL_KEY))
...
pub trait ScopeObjectMatch: ScopeObject {
	...
	///
	/// This will be borrowed, so if you want to match on a `&str` this type
	/// should be `str`.
	type Input: ?Sized;
...
mod tests {
	...
	fn denied_command_takes_precendence() {
		...
		assert!(authority.resolve_access(command, window, webview, &Origin::Local).is_none());
	}
	...
	fn resolve_access_message() {
		...
		let allowed_commands = [
			...
			(
				...
				vec![resolved_webview_window_cmd, resolved_webview_window_remote_cmd],
			),
		]
		.into_iter()
		.collect();
