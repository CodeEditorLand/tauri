...
use crate::{ipc::InvokeError, sealed::ManagerBase, AppHandle, Manager, Runtime, StateManager};
...
impl RuntimeAuthority {
	...
	pub fn new(acl: BTreeMap<String, Manifest>, resolved_acl: Resolved) -> Self {
		let command_cache =
			resolved_acl.command_scope.keys().map(|key| (*key, StateManager::new())).collect();
		Self {
			acl,
