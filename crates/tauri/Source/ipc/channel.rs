...
impl<'de, R: Runtime, TSend: Clone> CommandArg<'de, R> for Channel<TSend> {
	...
	fn from_command(command: CommandItem<'de, R>) -> Result<Self, InvokeError> {
		...
		JavaScriptChannelId::from_str(&value).map(|id| id.channel_on(webview)).map_err(|_| {
...
pub fn plugin<R: Runtime>() -> TauriPlugin<R> {
	PluginBuilder::new(CHANNEL_PLUGIN_NAME).invoke_handler(crate::generate_handler![fetch]).build()
}
