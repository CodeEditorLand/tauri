...
#[command(root = "crate")]
pub fn unlisten<R:Runtime>(webview:Webview<R>, event:EventName, event_id:EventId) -> Result<()> {
	webview.unlisten_js(&event, event_id)
}

#[command(root = "crate")]
pub fn emit<R:Runtime>(app:AppHandle<R>, event:EventName, payload:Option<JsonValue>) -> Result<()> {
