...
enum Pending {
	Unlisten(EventId),
	Listen { id:EventId, event:String, handler:Handler },
	Emit(EmitArgs),
}
...
impl Handler {
	fn new<F:Fn(Event) + Send + 'static>(target:EventTarget, callback:F) -> Self {
		Self { target, callback:Box::new(callback) }
	}
}
...
impl JsHandler {
	fn new(target:EventTarget, id:EventId) -> Self { Self { target, id } }
}
...
impl Listeners {
	...
	pub(crate) fn function_name(&self) -> &str { self.inner.function_name }

	/// Listener object name to represent the JavaScript event listener object.
	pub(crate) fn listeners_object_name(&self) -> &str { self.inner.listeners_object_name }
	...
	pub(crate) fn unlisten(&self, id:EventId) {
		match self.inner.handlers.try_lock() {
			...
			Ok(mut lock) => {
				lock.values_mut().for_each(|handler| {
					handler.remove(&id);
				})
			},
		}
	...
	where
		R: Runtime,
		I: Iterator<Item = &'a Webview<R>>, {
		self.emit_js_filter(webviews, event, emit_args, None::<&dyn Fn(&EventTarget) -> bool>)
	}
}
