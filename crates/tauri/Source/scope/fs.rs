...
fn push_pattern<P:AsRef<Path>, F:Fn(&str) -> Result<Pattern, glob::PatternError>>(
	...
	#[cfg(windows)]
	{
		...
		if is_unc {
			...
			// `dunce::simplified` will check that and return `path` as is without
			// simplification
			let simplified =
				path.to_str().and_then(|s| s.get(4..)).map_or(path.as_path(), Path::new);
	...
	// Add canonicalized version of the pattern or canonicalized version of its
	// parents so `/data/user/0/appid/assets/*` would be canonicalized to
	// `/data/data/appid/assets/*` and can then be matched against any of them.
...
/// Attempt to canonicalize path or its parents in case we have a path like
/// `/data/user/0/appid/**` where `**` obviously does not exist but we need to
...
///
/// the second iteration needs to check `/data/user/0/appid/assets` and save the
/// `*` component to append later.
///
/// if it also does not exist, a third iteration is required to check
/// `/data/user/0/appid` with `assets/*` as the cached value (`checked_path`
/// variable) on Android that gets canonicalized to `/data/data/appid` so the
/// final value will be `/data/data/appid/assets/*` which is the value we want
/// to check when we execute the `Scope::is_allowed` function
fn canonicalize_parent(mut path:PathBuf) -> Option<PathBuf> {
	let mut failed_components = None;

	loop {
		if let Ok(path) = path.canonicalize() {
			break Some(if let Some(p) = failed_components { path.join(p) } else { path });
		}
...
impl Scope {
	...
	pub fn unlisten(&self, id:ScopeEventId) { self.event_listeners.lock().unwrap().remove(&id); }
	...
	///
	/// Returns `false` if the path was explicitly forbidden or neither allowed
	/// nor forbidden.
	///
...
fn escaped_pattern_with(p:&str, append:&str) -> Result<Pattern, glob::PatternError> {
	if p.ends_with(MAIN_SEPARATOR) {
		...
		Pattern::new(&format!("{}{}{append}", glob::Pattern::escape(p), MAIN_SEPARATOR))
	}
}
...
mod tests {
	...
	fn windows_root_paths() {
		...
		let scope = new_scope();
		{
			...
			scope.allow_directory("\\\\?\\UNC\\localhost\\c$", true).unwrap();
			assert!(scope.is_allowed("\\\\localhost\\c$"));
			assert!(scope.is_allowed("\\\\localhost\\c$\\Windows"));
			assert!(scope.is_allowed("\\\\?\\UNC\\localhost\\c$\\Windows\\NonExistentFile"));
			// A non-existent file cannot be canonicalized to a verbatim UNC path, so this
			// will fail to match
		...
		{
			// Verbatim disk
			scope.allow_directory(format!("\\\\?\\{}", disk), true).unwrap();
			assert!(scope.is_allowed(format!("{}Cargo.toml", disk)));
			assert!(scope.is_allowed(cwd.join("Cargo.toml")));
			assert!(!scope.is_allowed("C:\\Windows"));
			assert!(!scope.is_allowed("Q:Cargo.toml"));
