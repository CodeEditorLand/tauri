...
//! Image types used by this crate and also referenced by the JavaScript API
//! layer.
...
impl Image<'static> {
	...
	///
	/// Similar to [`Self::new`] but avoids cloning the rgba data to get an
	/// owned Image.
	pub const fn new_owned(rgba:Vec<u8>, width:u32, height:u32) -> Self {
		Self { rgba:Cow::Owned(rgba), width, height }
	}
}
...
impl<'a> Image<'a> {
	...
	pub const fn new(rgba:&'a [u8], width:u32, height:u32) -> Self {
		Self { rgba:Cow::Borrowed(rgba), width, height }
	}
	...
	pub fn from_bytes(bytes:&[u8]) -> crate::Result<Self> {
		...
		let pixels = img.pixels().flat_map(|(_, _, pixel)| pixel.0).collect::<Vec<_>>();
		Ok(Self { rgba:Cow::Owned(pixels), width:img.width(), height:img.height() })
	}
	...
	/// bottom.
	pub fn rgba(&'a self) -> &'a [u8] { &self.rgba }
...
impl<'a> From<Image<'a>> for crate::runtime::Icon<'a> {
	fn from(img:Image<'a>) -> Self { Self { rgba:img.rgba, width:img.width, height:img.height } }
}
...
/// An image type that accepts file paths, raw bytes, previously loaded images
/// and image objects. This type is meant to be used along the [transformImage](https://v2.tauri.app/reference/javascript/api/namespaceimage/#transformimage) API.
///
/// # Stability
///
/// The stability of the variants are not guaranteed, and matching against them
/// is not recommended. Use [`JsImage::into_img`] instead.
...
pub enum JsImage {
	...
	#[non_exhaustive]
	Bytes(Vec<u8>),
	/// An image that was previously loaded with the API and is stored in the
	/// resource table.
	#[non_exhaustive]
	Resource(ResourceId),
...
impl JsImage {
	...
	///
	/// This will retrieve the image from the passed [`ResourceTable`] if it is
	/// [`JsImage::Resource`] and will return an error if it doesn't exist in
	/// the passed [`ResourceTable`] so make sure the passed [`ResourceTable`]
	/// is the same one used to store the image, usually this should be
