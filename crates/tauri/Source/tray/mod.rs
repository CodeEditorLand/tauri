...
impl From<tray_icon::TrayIconEvent> for TrayIconEvent {
	fn from(value:tray_icon::TrayIconEvent) -> Self {
		match value {
			tray_icon::TrayIconEvent::Click { id, position, rect, button, button_state } => {
				TrayIconEvent::Click {
					id,
					position,
					rect:Rect { position:rect.position.into(), size:rect.size.into() },
					button:button.into(),
					button_state:button_state.into(),
				}
			},
			tray_icon::TrayIconEvent::DoubleClick { id, position, rect, button } => {
...
impl<R:Runtime> TrayIconBuilder<R> {
	...
	pub fn id(&self) -> &TrayIconId { self.inner.id() }
...
impl<R:Runtime> Clone for TrayIcon<R> {
	fn clone(&self) -> Self {
		Self { id:self.id.clone(), inner:self.inner.clone(), app_handle:self.app_handle.clone() }
	}
}
...
impl<R:Runtime> TrayIcon<R> {
	fn register(
		...
		if let Some(handler) = on_menu_event {
			app_handle.manager.menu.global_event_listeners.lock().unwrap().push(handler);
		}
		...
		app_handle.manager.tray.icons.lock().unwrap().push(self.clone());
	}

	/// The application handle associated with this type.
	pub fn app_handle(&self) -> &AppHandle<R> { &self.app_handle }
	...
	pub fn set_icon_as_template(&self, #[allow(unused)] is_template:bool) -> crate::Result<()> {
		#[cfg(target_os = "macos")]
		run_item_main_thread!(self, |self_:Self| self_.inner.set_icon_as_template(is_template))?;
		Ok(())
	}
	...
	pub fn set_show_menu_on_left_click(&self, #[allow(unused)] enable:bool) -> crate::Result<()> {
		#[cfg(target_os = "macos")]
		run_item_main_thread!(self, |self_:Self| self_.inner.set_show_menu_on_left_click(enable))?;
		Ok(())
	}
	...
	pub fn rect(&self) -> crate::Result<Option<crate::Rect>> {
		run_item_main_thread!(self, |self_:Self| {
			self_
				.inner
				.rect()
				.map(|rect| Rect { position:rect.position.into(), size:rect.size.into() })
		})
	}
}

impl<R:Runtime> Resource for TrayIcon<R> {
	fn close(self: std::sync::Arc<Self>) { self.app_handle.remove_tray_by_id(&self.id); }
}
...
mod tests {
	...
	fn tray_event_json_serialization() {
		// NOTE: if this test is ever changed, you probably need to change
		// `TrayIconEvent` in JS as well
