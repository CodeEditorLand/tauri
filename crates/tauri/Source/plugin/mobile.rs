...
pub fn send_channel_data(
	...
	if let Some(channel) =
		CHANNELS.get_or_init(Default::default).lock().unwrap().get(&(channel_id as u32))
	{
		...
		if let Some(webview) = self.handle.manager.webviews().values().next() {
			...
			unsafe {
				crate::ios::register_plugin(
					&self.name.into(),
					init_fn(),
					&serde_json::to_string(&self.raw_config).unwrap().as_str().into(),
					std::ptr::null(),
				)
			};
		}
...
>(
	...
	fn run<R:Runtime>(
		...
		env.call_method(
			...
			&[id.into(), (&plugin).into(), (&command).into(), (&data).into()],
		)?;
