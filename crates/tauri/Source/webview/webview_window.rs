...
use crate::{
	...
	WindowEvent,
	event::EventTarget,
	ipc::{CommandArg, CommandItem, InvokeError, OwnedInvokeResponder, ScopeObject},
	manager::AppManager,
...
impl<'a, R:Runtime, M:Manager<R>> WebviewWindowBuilder<'a, R, M> {
	...
	/// Initializes a webview window builder from a [`WindowConfig`] from
	...
	/// the label of the new [`WebviewWindowBuilder`].
	///
	/// # Known issues
	///
	/// On Windows, this function deadlocks when used in a synchronous command,
	...
	/// 		handle,
	/// 		&[&Submenu::with_items(handle, "File", true, &[&save_menu_item])?],
	/// 	)?;
	/// 	let webview_window = tauri::WebviewWindowBuilder::new(
	/// 		app,
	/// 		"editor",
	...
	/// such as a development server, but it might be implemented in the
	/// future. **Always** check the request URL.
	///
	/// # Examples
	/// ```rust,no_run
	...
	/// Defines a closure to be executed when the webview navigates to a URL.
	...
	/// Set a download event handler to be notified when a download is requested
	/// or finished.
	///
	/// Returning `false` prevents the download from happening on a
	/// [`DownloadEvent::Requested`] event.
	///
	/// # Examples
	#[cfg_attr(
		feature = "unstable",
...
```
  "####
	...
	/// [`tauri_runtime::webview::PageLoadEvent::Started`] if the page has
	/// started loading or [`tauri_runtime::webview::PageLoadEvent::Finished`]
	...
	pub fn on_page_load<F:Fn(WebviewWindow<R>, PageLoadPayload<'_>) + Send + Sync + 'static>(
		...
		self.webview_builder = self.webview_builder.on_page_load(move |webview, payload| {
			f(WebviewWindow { window:webview.window(), webview }, payload)
		});
...
impl<R:Runtime, M:Manager<R>> WebviewWindowBuilder<'_, R, M> {
	...
	/// Adds the provided JavaScript to a list of scripts that should be run
	/// after the global object has been created, but before the HTML document
	/// has been parsed and before any other script included by the HTML
	/// document is run.
	///
	/// Since it runs on all top-level document and child frame page
	/// navigations, it's recommended to check the `window.location` to guard
	/// your script from running on unexpected origins.
	///
	/// # Examples
	///
	/// ```rust
	...
	/// so if you use this method, you also need to disable these components by
	/// yourself if you want.
	#[must_use]
	pub fn additional_browser_args(mut self, additional_args:&str) -> Self {
		self.webview_builder = self.webview_builder.additional_browser_args(additional_args);
	...
	/// Sets the webview to automatically grow and shrink its size and position
	...
	/// this path should be unpacked Chrome extensions on Windows, and compiled
	/// `.so` extensions on Linux.
	///
	/// ## Platform-specific:
	///
	...
	pub fn data_store_identifier(mut self, data_store_identifier:[u8; 16]) -> Self {
		self.webview_builder = self.webview_builder.data_store_identifier(data_store_identifier);
		self
	}
	...
	/// Whether web inspector, which is usually called browser devtools, is
...
impl<R:Runtime> AsRef<Webview<R>> for WebviewWindow<R> {
	fn as_ref(&self) -> &Webview<R> { &self.webview }
}

impl<R:Runtime> Clone for WebviewWindow<R> {
	fn clone(&self) -> Self { Self { window:self.window.clone(), webview:self.webview.clone() } }
}

impl<R:Runtime> Eq for WebviewWindow<R> {}
impl<R:Runtime> PartialEq for WebviewWindow<R> {
	/// Only use the [`Window`]'s label to compare equality.
	fn eq(&self, other:&Self) -> bool { self.webview.eq(&other.webview) }
}
...
impl<'de, R:Runtime> CommandArg<'de, R> for WebviewWindow<R> {
	...
	fn from_command(command:CommandItem<'de, R>) -> Result<Self, InvokeError> {
		...
		if window.is_webview_window() {
			return Ok(Self { window:window.clone(), webview });
		}
...
impl<R:Runtime> WebviewWindow<R> {
	/// Initializes a [`WebviewWindowBuilder`] with the given window label and
	/// webview URL.
	///
	...
	/// Resolves the given command scope for this webview on the currently
	/// loaded URL.
	///
	/// If the command is not allowed, returns None.
	///
	/// If the scope cannot be deserialized to the given type, an error is
	/// returned.
	///
	/// In a command context this can be directly resolved from the command
	/// arguments via [crate::ipc::CommandScope]:
	///
	/// ```
...
impl<R:Runtime> WebviewWindow<R> {
	...
	pub fn menu(&self) -> Option<Menu<R>> { self.window.menu() }
	...
	pub fn remove_menu(&self) -> crate::Result<Option<Menu<R>>> { self.window.remove_menu() }

	/// Hides the window menu.
	pub fn hide_menu(&self) -> crate::Result<()> { self.window.hide_menu() }

	/// Shows the window menu.
	pub fn show_menu(&self) -> crate::Result<()> { self.window.show_menu() }

	/// Shows the window menu.
	pub fn is_menu_visible(&self) -> crate::Result<bool> { self.window.is_menu_visible() }
...
impl<R:Runtime> WebviewWindow<R> {
	...
	pub fn scale_factor(&self) -> crate::Result<f64> { self.window.scale_factor() }
	...
	/// and borders.
	pub fn inner_size(&self) -> crate::Result<PhysicalSize<u32>> { self.window.inner_size() }
	...
	pub fn outer_size(&self) -> crate::Result<PhysicalSize<u32>> { self.window.outer_size() }

	/// Gets the window's current fullscreen state.
	pub fn is_fullscreen(&self) -> crate::Result<bool> { self.window.is_fullscreen() }

	/// Gets the window's current minimized state.
	pub fn is_minimized(&self) -> crate::Result<bool> { self.window.is_minimized() }

	/// Gets the window's current maximized state.
	pub fn is_maximized(&self) -> crate::Result<bool> { self.window.is_maximized() }

	/// Gets the window's current focus state.
	pub fn is_focused(&self) -> crate::Result<bool> { self.window.is_focused() }

	/// Gets the window's current decoration state.
	pub fn is_decorated(&self) -> crate::Result<bool> { self.window.is_decorated() }

	/// Gets the window's current resizable state.
	pub fn is_resizable(&self) -> crate::Result<bool> { self.window.is_resizable() }

	/// Whether the window is enabled or disabled.
	pub fn is_enabled(&self) -> crate::Result<bool> { self.webview.window().is_enabled() }
	...
	///
	/// - **Linux / iOS / Android:** Unsupported.
	pub fn is_maximizable(&self) -> crate::Result<bool> { self.window.is_maximizable() }
	...
	///
	/// - **Linux / iOS / Android:** Unsupported.
	pub fn is_minimizable(&self) -> crate::Result<bool> { self.window.is_minimizable() }
	...
	///
	/// - **Linux / iOS / Android:** Unsupported.
	pub fn is_closable(&self) -> crate::Result<bool> { self.window.is_closable() }

	/// Gets the window's current visibility state.
	pub fn is_visible(&self) -> crate::Result<bool> { self.window.is_visible() }

	/// Gets the window's current title.
	pub fn title(&self) -> crate::Result<String> { self.window.title() }
	...
	#[cfg(target_os = "macos")]
	pub fn ns_window(&self) -> crate::Result<*mut std::ffi::c_void> { self.window.ns_window() }

	/// Returns the pointer to the content view of this window.
	#[cfg(target_os = "macos")]
	pub fn ns_view(&self) -> crate::Result<*mut std::ffi::c_void> { self.window.ns_view() }

	/// Returns the native handle that is used by this window.
	#[cfg(windows)]
	pub fn hwnd(&self) -> crate::Result<HWND> { self.window.hwnd() }
	...
	pub fn gtk_window(&self) -> crate::Result<gtk::ApplicationWindow> { self.window.gtk_window() }
	...
	pub fn default_vbox(&self) -> crate::Result<gtk::Box> { self.window.default_vbox() }
	...
	///
	/// - **macOS**: Only supported on macOS 10.14+.
	pub fn theme(&self) -> crate::Result<crate::Theme> { self.window.theme() }
}
...
impl<R:Runtime> WebviewWindow<R> {
	/// Centers the window.
	pub fn center(&self) -> crate::Result<()> { self.window.center() }
	...
	pub fn set_title(&self, title:&str) -> crate::Result<()> { self.window.set_title(title) }

	/// Maximizes this window.
	pub fn maximize(&self) -> crate::Result<()> { self.window.maximize() }

	/// Un-maximizes this window.
	pub fn unmaximize(&self) -> crate::Result<()> { self.window.unmaximize() }

	/// Minimizes this window.
	pub fn minimize(&self) -> crate::Result<()> { self.window.minimize() }

	/// Un-minimizes this window.
	pub fn unminimize(&self) -> crate::Result<()> { self.window.unminimize() }

	/// Show this window.
	pub fn show(&self) -> crate::Result<()> { self.window.show() }

	/// Hide this window.
	pub fn hide(&self) -> crate::Result<()> { self.window.hide() }
	...
	///     Windows 11, it will have a rounded corners.
	/// - **Linux:** Unsupported.
	pub fn set_shadow(&self, enable:bool) -> crate::Result<()> { self.window.set_shadow(enable) }
	...
	/// Requires the window to be transparent.
	///
	/// See [`crate::window::EffectsBuilder`] for a convenient builder for
	/// [`crate::utils::config::WindowEffectsConfig`].
	///
	///
	/// ```rust,no_run
	/// use tauri::{
	...
	pub fn set_focus(&self) -> crate::Result<()> { self.window.set_focus() }

	/// Sets this window' icon.
	pub fn set_icon(&self, icon:Image<'_>) -> crate::Result<()> { self.window.set_icon(icon) }
	...
	///     be ignored for the webview layer..
	///   - On Windows 8 and newer: translucent colors are not supported so any
	///     alpha value other than `0` will be replaced by `255` for the webview
	///     layer.
	...
	pub fn start_dragging(&self) -> crate::Result<()> { self.window.start_dragging() }

	/// Sets the overlay icon on the taskbar **Windows only**. Using `None` will
	/// remove the icon
	///
	...
	///
	/// ## Platform-specific
	/// - **Windows:** Unsupported, use [`WebviewWindow::set_overlay_icon`]
	///   instead.
	/// - **iOS:** iOS expects i32, the value will be clamped to i32::MIN,
	///   i32::MAX.
	/// - **Android:** Unsupported.
	...
	/// Sets the taskbar badge label **macOS only**. Using `None` will remove
	/// the badge
	#[cfg(target_os = "macos")]
...
impl<R:Runtime> WebviewWindow<R> {
	...
	pub fn print(&self) -> crate::Result<()> { self.webview.print() }
}

/// Webview APIs.
impl<R:Runtime> WebviewWindow<R> {
	/// Executes a closure, providing it with the webview handle that is
	...
	pub fn url(&self) -> crate::Result<Url> { self.webview.url() }

	/// Navigates the webview to the defined url.
	pub fn navigate(&mut self, url:Url) -> crate::Result<()> { self.webview.navigate(url) }
	...
	pub fn eval(&self, js:&str) -> crate::Result<()> { self.webview.eval(js) }
	...
	/// ## Platform-specific
	///
	/// - **macOS:** Only supported on macOS 10.15+. This is a private API on
	///   macOS, so you cannot use this if your application will be published on
	///   the App Store.
	///
	/// # Examples
	///
...
impl<R:Runtime> Listener<R> for WebviewWindow<R> {
	...
	where
		...
		self.manager().listen(
			event.into(),
			EventTarget::WebviewWindow { label:self.label().to_string() },
			handler,
		)
	}
...
impl<R:Runtime> Manager<R> for WebviewWindow<R> {
	fn resources_table(&self) -> MutexGuard<'_, ResourceTable> {
		self.webview.resources_table.lock().expect("poisoned window resources table")
	}
}

impl<R:Runtime> ManagerBase<R> for WebviewWindow<R> {
	fn manager(&self) -> &AppManager<R> { self.webview.manager() }

	fn manager_owned(&self) -> Arc<AppManager<R>> { self.webview.manager_owned() }

	fn runtime(&self) -> RuntimeOrDispatch<'_, R> { self.webview.runtime() }

	fn managed_app_handle(&self) -> &AppHandle<R> { self.webview.managed_app_handle() }
}
