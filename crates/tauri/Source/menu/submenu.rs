...
impl<R:Runtime> ContextMenuBase for Submenu<R> {
	...
	fn inner_context(&self) -> &dyn muda::ContextMenu { (*self.0).as_ref() }
...
impl<R:Runtime> Submenu<R> {
	...
	/// Creates a new menu with given `items`. It calls [`Submenu::new`] and
	...
	pub(crate) fn inner(&self) -> &muda::Submenu { (*self.0).as_ref() }

	/// The application handle associated with this type.
	pub fn app_handle(&self) -> &AppHandle<R> { &self.0.app_handle }

	/// Returns a unique identifier associated with this submenu.
	pub fn id(&self) -> &MenuId { &self.0.id }
	...
	/// Add menu items to the beginning of this submenu. It calls
	/// [`Submenu::insert_items`] with position of `0` internally.
	...
	where
		...
		self.items().unwrap_or_default().into_iter().find(|i| i.id() == &id)
	}
