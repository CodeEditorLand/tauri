...
const PLUGINS: &[(&str, &[(&str, bool)])] = &[
	...
	("core:event", &[("listen", true), ("unlisten", true), ("emit", true), ("emit_to", true)]),
	(
		"core:window",
		&[
			("create", false),
	...
	(
		"core:image",
		&[("new", true), ("from_bytes", true), ("from_path", true), ("rgba", true), ("size", true)],
	),
...
permissions = [{}]
"#,
		PLUGINS.iter().map(|(k, _)| format!("- '{k}:default'")).collect::<Vec<_>>().join("\n"),
		PLUGINS.iter().map(|(k, _)| format!("'{k}:default'")).collect::<Vec<_>>().join(",")
	);
