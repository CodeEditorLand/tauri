...
fn resource_dir_from<P:AsRef<std::path::Path>>(
	...
	// executable in a development machine We check if the binary is inside a
	// `target` folder which can be either `target/$profile` or
	// `target/$triple/$profile` and see if there's a .cargo-lock file along the
	// executable This ensures the check is safer so it doesn't affect apps in
	// production Windows also includes the resources in the executable folder so
	// we check that too
	...
	#[cfg(target_os = "linux")]
	{
		...
		res = if let Ok(bundle_dir) =
			exe_dir.join(format!("../lib/{}", package_info.name)).canonicalize()
		{
			Ok(bundle_dir)
		} else if let Some(appdir) = &env.appdir {
			let appdir:&std::path::Path = appdir.as_ref();
			Ok(PathBuf::from(format!("{}/usr/lib/{}", appdir.display(), package_info.name)))
		} else {
			// running bundle
