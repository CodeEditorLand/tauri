...
impl From<AssetKey> for String {
	fn from(key:AssetKey) -> Self { key.0 }
}
...
impl<P:AsRef<Path>> From<P> for AssetKey {
	fn from(path:P) -> Self {
		// TODO: change this to utilize `Cow` to prevent allocating an
		// intermediate `PathBuf` when not necessary
...
pub struct EmbeddedAssets {
	...
	// Hashes that are associated to the CSP of the HTML file identified by the
	// map key (the HTML asset key).
...
impl EmbeddedAssets {
	...
	pub const fn new(
		...
		Self { assets:map, global_hashes, html_hashes }
	}
