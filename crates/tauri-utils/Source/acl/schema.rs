...
impl<'a>
	...
	> for Manifest
{
	fn has_default_permission_set(&self) -> bool { self.default_permission.is_some() }

	fn default_set_description(&self) -> Option<&str> {
		self.default_permission.as_ref().map(|d| d.description.as_str())
	}
...
impl<'a> PermissionSchemaGenerator<'a, Iter<'a, PermissionSet>, Iter<'a, Permission>>
	for PermissionFile
{
	fn has_default_permission_set(&self) -> bool { self.default.is_some() }

	fn default_set_description(&self) -> Option<&str> {
		self.default.as_ref().and_then(|d| d.description.as_deref())
	}

	fn permission_sets(&'a self) -> Iter<'a, PermissionSet> { self.set.iter() }

	fn permissions(&'a self) -> Iter<'a, Permission> { self.permission.iter() }
}

/// Collect and include all possible identifiers in `Identifier` defintion in
/// the schema
...
/// Collect permission schemas and its associated scope schema and schema
/// definitons from plugins and replace `PermissionEntry` extend object syntax
/// with a new schema that does conditional checks to serve the relavent scope
/// schema for the right permissions schema, in a nutshell, it
...
/// Extend schema with collected permissions from the passed
/// [`PermissionFile`]s.
