...
impl Resolved {
	...
	pub fn resolve(
		...
		let resolved = Self { allowed_commands, denied_commands, command_scope, global_scope };
...
/// Iterate over permissions in a capability, resolving permission sets if
/// necessary to produce a [`ResolvedPermission`] and calling the provided
/// callback with it.
fn with_resolved_permissions<F:FnMut(ResolvedPermission<'_>) -> Result<(), Error>>(
	...
	for permission_entry in &capability.permissions {
		...
		for TraversedPermission { key, permission_name, permission } in permissions {
...
fn get_permission_set_permissions<'a>(
	...
	for perm in &set.permissions {
		...
		let (manifest, permission_id, key, permission_name) = if let Some((new_key, manifest)) =
			id.get_prefix().and_then(|k| acl.get(k).map(|m| (k, m)))
		{
		...
		if permission_name == "default" {
			permissions.extend(
				manifest
					...
					.as_ref()
					.map(|default| {
						get_permission_set_permissions(permission_id, acl, manifest, default)
					})
					.transpose()?
					.unwrap_or_default(),
			);
...
fn display_perm_key(prefix:&str) -> &str {
	if prefix == APP_ACL_KEY { "app manifest" } else { prefix }
}
...
mod tests {
	...
	fn manifest<const P: usize, const S: usize>(
		...
		(
			name.to_string(),
			Manifest {
				default_permission:default_set.map(|perms| {
					PermissionSet {
				...
				permissions:permissions
					...
					.map(|p| {
						(
							p.to_string(),
							Permission { identifier:p.to_string(), ..Default::default() },
						)
					})
					.collect(),
	...
	fn id(id:&str) -> Identifier { Identifier::try_from(id.to_string()).unwrap() }

	#[test]
	fn resolves_permissions_from_other_plugins() {
		let acl = [
			manifest("fs", ["read", "write", "rm", "exist"], Some(&["read", "exist"]), []),
			manifest(
				"http",
				["fetch", "fetch-cancel"],
				None,
			...
			manifest(
				...
				[(
					"extra",
					&["save", "fs:default", "fs:write", "http:default", "http:fetch-with-cancel"],
				)],
			),
		]
		.into();
