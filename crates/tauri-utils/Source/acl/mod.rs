...
impl Permission {
	...
	pub fn is_active(&self, target: &Target) -> bool {
		self.platforms.as_ref().map(|platforms| platforms.contains(target)).unwrap_or(true)
	}
}
...
impl RemoteUrlPattern {
	...
	pub fn test(&self, url: &Url) -> bool {
		self.0.test(urlpattern::UrlPatternMatchInput::Url(url.clone())).unwrap_or_default()
	}
}
