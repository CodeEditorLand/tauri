...
impl<'a> ResourcePaths<'a> {
	...
	pub fn iter(self) -> ResourcePathsIter<'a> { self.iter }
}
...
impl<'a> ResourcePathsIter<'a> {
	...
	fn next_current_path(&mut self) -> Option<crate::Result<Resource>> {
		// should be safe to unwrap since every call to
		// `self.next_current_path()` is preceeded with assignemt to
		// `self.current_path`
...
mod tests {
	...
	fn resource_paths_iter_slice_allow_walk() {
		...
		let expected = expected_resources(&[
			...
			("../src/assets/javascript.svg", "_up_/src/assets/javascript.svg"),
	...
	fn resource_paths_errors() {
		...
		let resources = ResourcePaths::from_map(
			&std::collections::HashMap::from_iter([
				...
				// exists but only contains directories and will not produce
				// any values
