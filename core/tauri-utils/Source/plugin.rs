...
mod build {
	...
	/// Known file name of the file that contains an array with the path of all
	/// API scripts defined with [`define_global_api_script_path`].
	...
	/// Collects the path of all the global API scripts defined with
	/// [`define_global_api_script_path`] and saves them to the out dir with
	/// filename [`GLOBAL_API_SCRIPT_FILE_LIST_PATH`].
