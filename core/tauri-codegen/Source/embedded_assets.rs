...
///
/// This is the compile time generation of [`tauri_utils::assets::Assets`] from
/// a directory. Assets from the directory are added as compiler dependencies by
/// dummy including the original, uncompressed assets.
///
...
impl AssetOptions {
	...
	/// Instruct the asset handler to inject the CSP token to HTML files (Linux
	/// only) and add asset nonces and hashes to the policy.
	#[must_use]
	...
	/// Instruct the asset handler to **NOT** modify the CSP. This is **NOT**
	/// recommended.
...
impl EmbeddedAssets {
	...
	pub fn new(
		...
		// we need to pre-compute all files now, so that we can inject data from
		// all files into a few
	...
	fn compression_settings() -> BrotliEncoderParams {
		...
		// the following compression levels are hand-picked and are not
		// min-maxed. they have a good balance of runtime vs size for the
		// respective profile goals. see the "brotli" section of this comment https://github.com/tauri-apps/tauri/issues/3571#issuecomment-1054847558
	...
	fn compress_file(
      ...
      .map_err(|_| EmbeddedAssetsError::PrefixInvalid {
		...
		// we must canonicalize the base of our paths to allow long paths on
		// windows
		...
		if !out_path.exists() {
			...
			let mut out_file = File::create(&out_path).map_err(|error| {
				EmbeddedAssetsError::AssetWrite { path:out_path.clone(), error }
			})?;
...
impl ToTokens for EmbeddedAssets {
	fn to_tokens(&self, tokens:&mut TokenStream) {
		...
		// we expect phf related items to be in path when generating the path
		// code
