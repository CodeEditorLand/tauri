...
//!
//! - Embed, hash, and compress assets, including icons for the app as well as
//!   the tray icon.
...
pub enum CodegenConfigError {
	...
	// this error should be "impossible" because we use
	// std::env::current_dir() - cover it anyways
...
/// Get the [`Config`] from the `TAURI_CONFIG` environmental variable, or read
...
pub fn get_config(path:&Path) -> Result<(Config, PathBuf), CodegenConfigError> {
	...
	// this should be impossible because of the use of `current_dir()` above,
	// but handle it anyways
	let parent = path
		.parent()
	...
	// in the future we may want to find a way to not need the TAURI_CONFIG env
	// var so that it is impossible for the content of two separate configs to
	...
	// crate.
	let mut config =
		serde_json::from_value(tauri_utils::config::parse::read_from(target, parent.clone())?)?;
...
///
/// Due to using a checksum as the filename, an existing file should be the
/// exact same content as the data being checked.
struct Cached {
	checksum:String,
}
