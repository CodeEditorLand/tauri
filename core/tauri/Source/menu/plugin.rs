...
impl CheckMenuItemPayload {
	pub fn create_item<R: Runtime>(self, webview: &Webview<R>) -> crate::Result<CheckMenuItem<R>> {
		...
		if let Some(handler) = self.handler {
			...
			webview.state::<MenuChannels>().0.lock().unwrap().insert(item.id().clone(), handler);
		}
...
fn items<R: Runtime>(
	...
	Ok(items.into_iter().map(|i| make_item_resource!(resources_table, i)).collect::<Vec<_>>())
}
...
async fn popup<R: Runtime>(
	...
	let window = window.map(|w| webview.manager().get_window(&w)).unwrap_or(Some(current_window));
