...
use super::{run_item_main_thread, MenuItem};
use crate::{
	menu::{MenuId, MenuItemInner},
	run_main_thread,
	AppHandle,
	Manager,
	Runtime,
...
impl<R:Runtime> MenuItem<R> {
	...
	where
		...
		let item = run_main_thread!(handle, || {
			...
			MenuItemInner { id:item.id().clone(), inner:Some(item), app_handle }
		})?;
	...
	where
		...
		let item = run_main_thread!(handle, || {
			...
			MenuItemInner { id, inner:Some(item), app_handle }
		})?;
