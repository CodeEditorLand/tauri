...
impl<R: Runtime> fmt::Debug for WindowManager<R> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("WindowManager").field("default_window_icon", &self.default_icon).finish()
	}
}
...
fn on_window_event<R: Runtime>(window: &Window<R>, event: &WindowEvent) -> crate::Result<()> {
	match event {
		...
		WindowEvent::Focused(focused) => window
			.emit_to_window(if *focused { WINDOW_FOCUS_EVENT } else { WINDOW_BLUR_EVENT }, ())?,
		WindowEvent::ScaleFactorChanged { scale_factor, new_inner_size, .. } => window
			.emit_to_window(
