...
use tauri_runtime::{
	...
	window::{DetachedWindow, DragDropEvent, PendingWindow, WindowBuilder},
};
...
impl<R:Runtime> WindowManager<R> {
	...
	pub(crate) fn attach_window(
		...
		let _ = window.run_on_main_thread(move || {
			manager.plugins.lock().expect("poisoned plugin store").window_created(window_);
		});

		window
	}

	pub fn labels(&self) -> HashSet<String> { self.windows_lock().keys().cloned().collect() }
}
...
impl<R:Runtime> Window<R> {
	...
	/// Checks whether has js listener for [`EventTarget::Window`] or
	/// [`EventTarget::WebviewWindow`]
...
fn on_window_event<R:Runtime>(window:&Window<R>, event:&WindowEvent) -> crate::Result<()> {
	match event {
		...
		WindowEvent::Focused(focused) => {
			window
				.emit_to_window(if *focused { WINDOW_FOCUS_EVENT } else { WINDOW_BLUR_EVENT }, ())?
		},
		WindowEvent::ScaleFactorChanged { scale_factor, new_inner_size, .. } => {
			window.emit_to_window(
				WINDOW_SCALE_FACTOR_CHANGED_EVENT,
				ScaleFactorChanged { scale_factor:*scale_factor, size:*new_inner_size },
			)?
		},
