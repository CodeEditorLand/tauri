...
// so we use the local IP address and we do not get a secure context without the
// custom protocol that proxies to the dev server additionally, we need the
// custom protocol to inject the initialization scripts on Android
// must also keep in sync with the `let mut response` assignment in
// prepare_uri_scheme_protocol
...
impl<R:Runtime> fmt::Debug for WebviewManager<R> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("WebviewManager")
			.field("invoke_initialization_script", &self.invoke_initialization_script)
...
impl<R:Runtime> WebviewManager<R> {
	pub(crate) fn register_uri_scheme_protocol<N:Into<String>>(
		...
		self.uri_scheme_protocols.lock().unwrap().insert(uri_scheme, protocol);
	}
	...
	fn prepare_pending_webview<M:Manager<R>>(
		...
		let pattern_init = PatternJavascript { pattern:(&*app_manager.pattern).into() }
      ...
      ",
		...
		for (uri_scheme, protocol) in &*self.uri_scheme_protocols.lock().unwrap() {
			...
			pending.register_uri_scheme_protocol(uri_scheme.clone(), move |p, responder| {
				(protocol.protocol)(&app_handle.lock().unwrap(), p, UriSchemeResponder(responder))
			});
		}
			...
			&& window_url.scheme() != "https"
		{
			...
			format!(
				...
				window_url.scheme(),
				host,
				window_url.port().map(|p| format!(":{p}")).unwrap_or_default()
			)
		} else {
			"null".into()
		};
		...
		if !registered_scheme_protocols.contains(&"ipc".into()) {
			let protocol =
				crate::ipc::protocol::get(manager.manager_owned(), pending.label.clone());
		...
		pending.on_page_load_handler.replace(Box::new(move |url, event| {
			...
			if let Some(w) = app_manager_.get_webview(&label) {
				...
				app_manager_.plugins.lock().unwrap().on_page_load(&w, &payload);
			}
		...
		if !registered_scheme_protocols.contains(&"asset".into()) {
			let asset_scope = app_manager.state().get::<crate::Scopes>().asset_protocol.clone();
		...
		#[cfg(feature = "isolation")]
		if let crate::Pattern::Isolation { assets, schema, key: _, crypto_keys } =
			&*app_manager.pattern
		{
	...
	pub fn prepare_webview<M:Manager<R>>(
		...
		if pending.webview_attributes.data_directory.is_none() {
			let local_app_data = manager
				.path()
				.resolve(&app_manager.config.identifier, crate::path::BaseDirectory::LocalData);
		...
		if pending.webview_attributes.zoom_hotkeys_enabled {
			...
			pending.webview_attributes.initialization_scripts.push(
				HotkeyZoom { os_name:std::env::consts::OS }
					.render_default(&Default::default())?
					.into_string(),
			)
		}
		...
		pending.navigation_handler = Some(Box::new(move |url| {
			// always allow navigation events for the isolation iframe and do not emit them
			// for consumers
			#[cfg(feature = "isolation")]
	...
	pub(crate) fn attach_webview(
		...
		{
			self.webviews_lock().insert(webview.label().to_string(), webview.clone());
		}
		...
		let webview_ = webview.clone();
		// run on main thread so the plugin store doesn't dead lock with the event loop
		// handler in App
		let _ = webview.run_on_main_thread(move || {
			manager.plugins.lock().expect("poisoned plugin store").webview_created(webview_);
		});
		...
		let _ = webview.manager.emit(
			...
			Some(crate::webview::CreatedEvent { label:webview.label().into() }),
		);
	...
	pub fn eval_script_all<S:Into<String>>(&self, script:S) -> crate::Result<()> {
		...
		self.webviews_lock().values().try_for_each(|webview| webview.eval(&script))
	}

	pub fn labels(&self) -> HashSet<String> { self.webviews_lock().keys().cloned().collect() }

	pub(crate) fn invoke_key(&self) -> &str { &self.invoke_key }
}

impl<R:Runtime> Webview<R> {
	/// Emits event to [`EventTarget::Window`] and
