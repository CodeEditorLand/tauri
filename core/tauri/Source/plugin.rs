...
pub trait Plugin<R:Runtime>: Send {
	...
	/// Add the provided JavaScript to a list of scripts that should be run
	...
	/// Callback invoked when webview tries to navigate to the given Url.
	/// Returning falses cancels navigation.
	#[allow(unused_variables)]
...
impl<R:Runtime> PluginHandle<R> {
	...
	pub fn app(&self) -> &AppHandle<R> { &self.handle }
}
...
impl<R:Runtime, C:DeserializeOwned> PluginApi<R, C> {
	...
	/// Gets the global scope defined on the permissions that are part of the
	/// app ACL.
...
///
/// This Builder offers a more concise way to construct Tauri plugins than
...
impl<R:Runtime, C:DeserializeOwned> Builder<R, C> {
	...
	/// It is recommended you use the [tauri::generate_handler] to generate the
	/// input to this method, as the input type is not considered stable yet.
	///
	/// # Examples
	///
	/// ```rust
	...
	/// Sets the provided JavaScript to be run after the global object has been
	/// created, but before the HTML document has been parsed and before any
	...
	/// instead of implicitly declared.
	///
	/// Note that calling this function multiple times overrides previous
	/// values.
	///
	/// # Examples
	///
	...
	/// Callback invoked when the webview tries to navigate to a URL. Returning
	...
	/// # Known limitations
	///
	/// URI scheme protocols are registered when the webview is created. Due to
	/// this limitation, if the plugin is registered after a webview has been
	/// created, this protocol won't be available.
	///
	/// # Arguments
	///
...
impl<R:Runtime> Default for PluginStore<R> {
	fn default() -> Self { Self { store:Vec::new() } }
}
...
impl<R:Runtime> PluginStore<R> {
	...
	pub(crate) fn on_event(&mut self, app:&AppHandle<R>, event:&RunEvent) {
		self.store.iter_mut().for_each(|plugin| plugin.on_event(app, event))
	}

	/// Runs the plugin `extend_api` hook if it exists. Returns whether the
	/// invoke message was handled or not.
	///
	/// The message is not handled when the plugin exists **and** the command
	/// does not.
...
pub enum PermissionState {
	...
	Denied,
	/// Permission must be requested, but you must explain to the user why your
	/// app needs that permission. **Android only**.
	PromptWithRationale,
