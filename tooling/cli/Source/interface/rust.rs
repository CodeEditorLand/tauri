...
impl Interface for Rust {
	...
	fn new(config:&Config, target:Option<String>) -> crate::Result<Self> {
		...
		std::env::set_var("IPHONEOS_DEPLOYMENT_TARGET", &config.bundle.ios.minimum_system_version);
	...
	fn get_binaries(&self, target:&str) -> crate::Result<Vec<BundleBinary>> {
		...
		if let Some(bins) = &self.cargo_settings.bin {
			...
			for bin in bins {
				...
				let is_main = bin.name == self.cargo_package_settings.name
					|| bin.name.as_str() == default_run;
		...
		if let Some(default_run) = self.package_settings.default_run.as_ref() {
			if !binaries.iter_mut().any(|bin| bin.name() == default_run) {
				binaries
					.push(BundleBinary::new(format!("{}{}", default_run, binary_extension), true));
			}
		}
