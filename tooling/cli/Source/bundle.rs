...
fn sign_updaters(
	...
	let pubkey =
		if maybe_path.exists() { std::fs::read_to_string(maybe_path)? } else { pubkey.to_string() };

	// if no password provided we use an empty string
	let password = std::env::var("TAURI_SIGNING_PRIVATE_KEY_PASSWORD").ok().or_else(|| {
		if ci {
			Some("".into())
		} else {
			None
	...
	let private_key =
		if maybe_path.exists() { std::fs::read_to_string(maybe_path)? } else { private_key };
