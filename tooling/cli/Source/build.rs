...
pub struct Options {
	...
	///
	/// It must be one of the values outputted by `$rustc --print target-list`
	/// or `universal-apple-darwin` for an universal macOS application.
	///
	...
	///
	/// Note that the `updater` bundle is not automatically added so you must
	/// specify it if the updater is enabled.
	#[clap(short, long, action = ArgAction::Append, num_args(0..), value_delimiter = ',')]
	pub bundles:Option<Vec<BundleFormat>>,
	/// Skip the bundling step even if `bundle > active` is `true` in tauri
	/// config.
	#[clap(long)]
	pub no_bundle:bool,
	...
	/// Command line arguments passed to the runner. Use `--` to explicitly
	/// mark the start of the arguments.
	pub args:Vec<String>,
